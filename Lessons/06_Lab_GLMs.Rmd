...
Can be applied to any data that has a continuous response, one categorical explanatory variable with two categories (or comparison to a single value if a one-sample test).
...
The null hypothesis must have the equality (ex: greater than / equal to).
...
> Answer: less than / equal to 50 is null hypothesis, hypothesis is greater than 50
...
#is this sample enough to make the assumption in the null hypothesis? 
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample
#the p-value is significant, so we reject the null hypothesis
#yes, we can prove that mu is less than 50 so it is within the threshold for a good standard
...
>the ozone levels are within the standard
...
Are Ozone levels different between 2018 and 2019? Similar to ANOVA
...
```{r}
#Exercise 2 (make sure to set Year as factor):
#1 
ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year)), na.rm = TRUE)+
  geom_density(alpha = 0.5)+
  scale_fill_manual(values = c("darkgreen", "darkblue"))+
  labs(x = "Ozone AQI", y = "Density", fill = "")
...
pmo3 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(pmo3)
#r^2 is very low
#but p-value is signficant 
#we want to add more variables to increase R^2
...
#omit NAs? 
EPAairAIC <- lm(data = EPAair, PM2.5 ~ Ozone + Month + 
              Year + meanLng + meanLat)
step(EPAairAIC)
summary(EPAairAIC) #R^2 improved just a bit, but isn't great
#maybe the relationships here are non-linear 
...
#this is the same as above regression 
```
