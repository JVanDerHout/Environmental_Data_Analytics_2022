...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #summary works with character entries with the last part of this import

colnames(NTL.phys.data) #checking that data was properly imported:
head(NTL.phys.data)
...
class(NTL.phys.data$sampledate)
head(NTL.phys.data$sampledate) #checking that years were assigned correctly
```
...
```{r, results = "hide"}
vignette("dplyr") #specifies what can happen to both rows and columns 
```
...
`!=`compare two values to see if they are different
`<` compare two values to see if one is greater
`<=` includes what comes after the equals 
`>` compare two values to see if one is lesser
`>=` includes what comes before the equals
`&` "and"
`|` "or"

```{r}
class(NTL.phys.data$lakeid) #factor
class(NTL.phys.data$depth) #factor
#can only compare like with like in filter function

# matrix filtering (to compare with dplyr down further)
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] 
#this subset of data is taken from the original data set
#here only the rows with a depth of 0 are being selected
#filtering with the comma after selects all of the columns and filters only by rows

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
#first argument is dataset, second argument is what you are trying to filter
#this does the same as above only collecting the surface data
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)
#this filters only depths less than 0.25 for a more specific surface information
...
#filter selects by rows, you need to specify which rows you are filtering for by values
#select is used to grab columns, or to rearrange them. selection is just by column name
summary(NTL.phys.data$lakename) #shows number of occurences of each lake name

NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") 
#use "or" because there is no observation for both lakes
...
#exclusion technique - selecting out every other lake and leaving only Paul and Peter Lakes

NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#helpful to use %in% when you need to select more than two elements and can't use ==

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
#these operations all perform the same function, written differently
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #here & and , perform the same function
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305)
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304) #these include the values = so going up and down one 
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #shortest selection
...
> Answer: Because values are what we are looking for and are more accurate

### Arrange

Arranging allows us to change the order of rows in our dataset. By default, the arrange function will arrange rows in ascending order. Descending order needs to be specified. 

```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #arranged in ascending depth
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #arrenged in descending depth
...
summary(NTL.phys.data.temp.desc)
...
#creating data subset by selecting the columns with lake name, and then everything from sample date to temperature 
...
#adds a new column with the temperature in F
#specifies orginal dataset and then what the new column is generatre from
#the name of the column is always left of the = sign in mutate
```
...
#extract just the month from the object
#this uses the date information from sampledate to adda  new column with the month 
...
#this uses the select function to move the columns around
#selects all the same columns and reorders them
...
#interval function creates interval object with specified start and stop dates
#used when data is organized by date
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) 
#first is first observed date and last is last observed date
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate))
#you can also calculate interval using "first" and "last" from dplyr here 
#this is a better way of finding your start and finish date in any size dataset
```
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% #entering original dataset
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #selecting lakes
  select(lakename, sampledate:temperature_C) %>% #selecting columns
  mutate(temperature_F = (temperature_C*9/5) + 32) #creating column with temp in F 
#pipe structure only requires you to enter dataset once 
...
```{r}
write.csv(NTL.phys.data.PeterPaul1, row.names = FALSE, file = "../../Environmental_Data_Analytics_2022/Data/Processed/NTL_LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv")

NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv
#save processed dataset in CSV for further reference
```
...
To keep long files seperate. Keep one code file for wrangling and another for analysis using the new CSV to create cleaner R scripts for reproducibility and sharing. 
