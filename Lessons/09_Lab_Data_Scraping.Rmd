...
#change the period to 24
#and change the site number to the site you want 

#Get the data, which starts on line 30
gage_data <- read.table(theURL2,skip = 28,header=TRUE,sep='\t') 
#skipping first 28 lines of info - everything with a #
#data are tab separated 
#read.table is base r for reading website data
...
nc_pop <- 
  get_acs(geography = "county",
          ...
          state = "NC",
          geometry = TRUE) #returning geometry allows you to get a spatial data frame

#you can map population below
...
#each vector holds a column of data 

#use selector gadget tool (ScrapeMate Beta on firefox) to fill html_nodes
...
df_withdrawals <- data.frame("State" = the_states,
                             "Price" = as.numeric(the_price),
                             "Capactiy" = as.numeric(gsub(",","",the_capacity)),
...
#constructing web address based on the year 
...
# ifelse is used to say that if the year is 2020, use first url, if it's not, then use the second custom web adress  
...
the_dataframes <- lapply(X = the_years, FUN = scrape.it) %>% bind_rows()
the_dataframes2 <- map(the_years, scrape.it) %>% bind_rows()
#creating one massive dataframe 
