...
#gets rid of other Lakes that don't have values but still show up in summary
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments)
class(NTL.nutrient.data.PeterPaul$sampledate)
#formatting dates again
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y")

#doing other operations with the pipe method 
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #inputting data
  mutate(month = month(sampledate)) %>% #adding month column
  select(lakeid:daynum, month, sampledate:comments) %>% # reorderring column order
  drop_na(depth) #dropping all NA values 
...
#selecting to keep all columns in range you want to keep
#removes lake ID 
...
## Gather and Spread (tidyr - also included in tidyverse)
...
#this is the equivalent of pivot in excel
#wider adds more columns
#longer adds more rows 

#gathering the nutrient data into one column:
#transform first into two columns
#1. category for nutrient
#2. value 
...
#assigns new values and creates "nutrient" column and "concentration" column 
#this replaces a column for each nutrient with two columns for all nutrients 
#gathered dataset will have more rows than the original dataset

#removing NAs in concentration column to make a more tidy dataset
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
#!is.na will return a logical operator to tell you if there are NA values and remove those values 
count(NTL.nutrient.data.PeterPaul.gathered$nutrient)
#R is counting how many observations there are for each nutrient type

#storing processed dataset on a file 
...
#trying to go back to the original layout of the data from 2 columns to 5 columns
...
#R is taking the names from column nutrient and values from the column concentration
...
#give column you want to seperate and the format you want it to be separated by (year, month day)
...
Only use rbind if all the rows are the same, or cbind if all the columns are the same. Otherwise, make sure to use the join function. 
...
```{r}
#full join function with two dataset
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset)
#tells you what the common columns of joining were used 
#full join is a good way to start if you aren't sure which join function to use 
...
```{r}
#less efficient way to import data
...
```{r}
#more efficient way to import multiple files 
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
#path is to the folder where the files are located
#pattern is how the file names end
#include full names 
LitterFiles
#This shows information on al 20 datasets and files 


#using a pipe to read each element using read.csv into one object
Litter <- LitterFiles %>%
  ldply(read.csv) #from plyr package
```
...
#same number of observations as in litter
#left join option
#increased number of columns resulting from both datasets 

dim(Litter)
dim(Trap)
dim(LitterTrap)

#selecting just a few columns of interest
LitterTrap <- LitterTrap %>%
  select(plotID:trapID, collectDate, functionalGroup:qaDryMass, subplotID:geodeticDatum)
#now there are only 13 variables 
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>% #just information on surface
  group_by(lakename, month) %>% #group by lake name and month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter NA for temp, tn_ug and tp_ug
  summarise(meantemp = mean(temperature_C), #create summaries for all of the below: 
...
#now view your print out with all of the above variables
NTL.PeterPaul.summaries
