...
#using facet_wrap by site name to create a plot for each site
PMplot.faceted <-
...
#using subset only to select sites of interest by year
PMplot.faceted2 <- 
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot - visualize relationships btwn two continuous datasers
lightvsDO <- 
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #reverse y axis for limnology
print(tempvsdepth)

#adding color by depth
#adding a trendline in ggplot 
NvsP <-
...
print(NvsP)
#geom_smooth adds trendline from data
#geom_abline you add your own trendline
...
#better than bar plot with error bars
Nutrientplot3 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # codes each nutrient as a different color 
print(Nutrientplot3)
...
# Violin plot - better for larger sample size
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) #
print(Nutrientplot5)

# Frequency polygons for different nutrients
# Using a tidy dataset
#axes need to be updated, and need legend
#don't do this
Nutrientplot6 <-
...
# Using a gathered dataset to better above plot gives automatic legend 
#do this instead
Nutrientplot7 <-   
