...
#Option 1 - Join the SVI attributes to the county spatial features
counties_sf_join <-  merge(x = counties_sf,
                           y = svi2018_nc_raw, 
                           by.x = "GEOID", 
                           by.y = "FIPS" )

#Option 2 - Tidyverse version of the join
counties_sf_join <- counties_sf %>% 
  left_join(svi2018_nc_raw, by = c("GEOID" = "FIPS") )

#View with mapview (prettier and more interactive)
...
EPAWindEnergy <- st_read('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv')

EPAWindEnergy$HUC_12 <- as.numeric(EPAWindEnergy$HUC_12)  
EPAWindEnergy$AvgWindEnergy <- as.numeric(EPAWindEnergy$AvgWindEnergy)  

EPAWindEnergy <- EPAWindEnergy %>% 
  mutate(WindEnergySum = sum(AvgWindEnergy)) %>% 
  mutate(HUC_8 = substr(HUC_12, 1,8)) %>%
  group_by(HUC_8)

#Join to HUC_8 features
huc8_sf_join = left_join(EPAWindEnergy, huc8_sf, by = "HUC_8")

#View the outputs
##this is not working for me 
#error: stat_sf requires the following missing aesthetics: geometry
ggplot(data = huc8_sf_join)+
  geom_sf(aes(fill=WindEnergySum))+
...
huc2_sf <- huc8_sf %>%
  select(SQ_MILES, ACRES, HUC_8, DWQ_Basin, geometry) %>% 
  group_by('DWQ_Basin')
  
#Summarize on DWQ Basin value
Watersheds <- Watersheds %>%  
  summarize(ACRES = sum(ACRES)) %>% 
  #summarize(SQ_MILES = sum(SQ_MILES)) - not found??
...
#option 1 Select intersecting counties using matrix subsetting
neuse_intersect_1 <- counties_sf_utm[neuse_sf,]

#option 2 Select intersecting counties using the `filter()` command
