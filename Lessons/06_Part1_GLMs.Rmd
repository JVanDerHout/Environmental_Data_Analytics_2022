...
GLMs are for comparing how variables affect other continuous variables. Assumes approximate normal distribution.
...
The GLMs are used for hypothesis testing. 
If pv-value is equal to or less than set number, reject null hypothesis. If p-value is greater, do not reject null hypothesis. Use the p-value in the f-statistics line. 
...
The goal for the linear regression is to find a **line of best fit**, which is the line drawn through the bivariate space that minimizes the total distance of points from the line. This is also called a "least squares" regression. The remainder of the variance not explained by the model is called the **residual error.** We want the residual series to follow a normal distribution and to have a mean 0 (on average we are looking for residuals to be 0 when running a linear regression). 

**P-values** one p-value from each coefficient and one p-value overall. 
...
Important components of the linear regression are the correlation and the R-squared value. The **correlation** is a number between -1 and 1, describing the relationship between the variables. Correlations close to -1 represent strong negative correlations, correlations close to zero represent weak correlations, and correlations close to 1 represent strong positive correlations. The **R-squared value** is the correlation squared, becoming a number between 0 and 1. The R-squared value describes the percent of variance accounted for by the explanatory variables. It tells you how much variability is explained by explanatory variables. Use adjusted R^2 when comparing the use of different variables in a model. 

**Adding Variables**
The more variables you have the more explanatory power you have. Adding variables also increases complexity and R^2 when the right variables are added.
Identifying new variables: testing for correlation between variables via a correlation matrix for correlations among all variables in the dataframe. 
...
#we provide it first with y, then x

# preferred way to format the lm() function
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression)
#p values are signficant 
#r^2 value gives explanatory power 

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
#You want to the mean of the error term to be zero to show you have a good linear regression
#you want R^2 to be as close to 1 as possible for highest explanatory power of the variables
...
>  There is a signficant negative correlation between irradiance and depth. The model explains 31% of the total variance. 
...
1. **Residuals vs. Fitted.** Scatter plot of the value predicted by the line of best fit is the fitted value, and the residual is the distance of that actual value from the predicted value. By definition, there will be a balance of positive and negative residuals. Watch for drastic asymmetry from side to side or a marked departure from zero for the red line - these are signs of a poor model fit.

2. **Normal Q-Q.** The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. Compares residuals close to normal distribution. 
...
#r^2 and p-values looking better with log transformed data

#better visualizations now
par(mfrow = c(2,2), mar=c(4,4,4,4))
...
When removing variables, reduction in AIC means variables can be removed from the model. If the AIC increases when a variable is removed, keep that variable! The algorithm will re-do the analysis with each one of the variable combinations. The best linear regression is the one with the lowest AIC. However you don't want two variables with high correlation in the same model, it can lead to overfitting. 
