...
---
Spatial analysis is really just a data frame with a geometry column. 
 ...
 *Raster parses landscape into a grid and is more like image data. Instead of a dataframe we are working with matrices. We aren't using raster data here because we don't have enough time.
 * Vector data model: points/lines/polygons
 * Spatial data structures: sfg, sfc, sf objects
 * Projections & Coordinate reference systems. Necessary to move flat calculations to the sphere of the earth. WGS-84 is most popular one. 
 * Visualization techniques
 *Geometry (geom) column shows the shapefile and coordinates, broken down into simple feature, simple feature geometry list column (sfc), and simple feature geometry (sfg)
...
#use st_read to read a shapefile
st_crs(counties_sf) #uses NAD83 CRS
#
...
st_crs(huc8_sf) #uses WGS 84 coordinate reference system
#counties uses NAD83 
...
WindEnergy <- st_read('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12'='factor')) %>% 
...
ggplot(data = huc8_sf_join)+
  ...
  labs(title = 'Wind Energy by HUC 8', fill = 'MW/year')
...
#now we have selected based on location

#***look up sp cheat sheet to get more info on all the things that can be done***

#Intersect the HUC_8s
#cookie batter & cookie cutter: clipping HUCs by counties
